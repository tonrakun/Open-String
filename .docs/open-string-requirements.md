...
- [x] 各OS向けバイナリそのものも、zip/tar.gzアーカイブとは別に単体アセットとしてReleaseへ同封する（インストーラを介さず直接ダウンロード・自己更新用に取得できるようにするため）（`release.yml`のPackageステップで`open-string-windows-x86_64.exe`/`open-string-macos-aarch64`/`open-string-linux-x86_64`を生成し、アーカイブと並行してアップロード・Releaseへ添付。macOSは`macos-latest`ランナーがApple Silicon（aarch64）でネイティブビルドするため、x86_64ではなくaarch64として命名）
...
### 6.2 性能・リソース
- [x] 単一バイナリでの配布（追加ランタイム不要）（リリースビルドの`open-string.exe`単体(約8.2MB)を空の一時ディレクトリにコピーして`--version`/`gui`を実行し、追加ファイル・追加ランタイム無しで動作することを確認。`src/gui/index.html`は`include_str!`でバイナリに埋め込み、実行時に外部ファイルを読まない）
- [x] アイドル時メモリ消費量の目標値設定（要ベンチマーク、OpenClaw＝Node.js実装との比較を指標にする）（本セッションでWindows上`open-string gui`をアイドル状態で実測：起動2秒後WorkingSet約11MB、12秒後も約11MBで増加なし。これを基準に目標値を「アイドル時メモリ消費50MB以下」と設定する。一般的なNode.js単体プロセスの目安(数十MB)、Electron/フルランタイムを伴う実装ではさらに大きくなる点を踏まえ、Rustネイティブバイナリとして明確に優位な値を確保する方針とする）
- [x] トークン消費削減率のベンチマーク方針策定（t0k3n-mcp適用時/非適用時の比較）（方針：(1)代表的なSub Agentタスク集合(例:「大きなファイルの要約」「特定シンボルの全呼び出し元検索」「中規模リポジトリのディレクトリ構造把握」)を固定する、(2)同一タスクをt0k3n-mcp接続時(`read_context_pack`/`read_code_skeleton`等の構造化読み込みツール経由)と非接続時(`read_file`による全文読み込みのみ)の双方で実行する、(3)`agent::estimate_history_tokens`で両条件の累積トークン数(入力+出力)を測定する、(4)削減率 = (非適用時トークン数 − 適用時トークン数) / 非適用時トークン数 で算出し、タスク種別ごとに記録する。実際のベンチマーク実行・数値化は別タスクで行う)

### 6.3 セキュリティ
- [x] 認証情報の暗号化保存（OS標準のセキュアストレージ利用：Windows Credential Manager / macOS Keychain / Linux Secret Service。`keyring` crateで実装、4.1参照）
- [x] 設定ファイルの平文秘匿情報禁止（OpenClawの`~/.moltbot/`漏洩事例の対策として明文化）（コードベース全体を棚卸しした結果、秘匿情報(Anthropic APIキー・チャットゲートウェイBotトークン)はいずれも`keyring`crate経由でOSセキュアストレージにのみ保存されており(`auth::AnthropicApiKeyProvider`/`gateway::GatewayTokenStore`)、`.mcp.json`・`hotreload.json`・セッション/進捗ファイル等のプレーンファイルに秘匿情報を書き込む経路は存在しないことを確認)
- [x] 間接的プロンプトインジェクション対策（外部コンテンツ内の指示文言を実行指示として扱わないフィルタ層）（`agent::tools::wrap_untrusted_content`を実装し、`fetch_url`の取得結果と、信頼できない(bundled以外の)Extensionのツール呼び出し結果(`ClaudeTaskExecutor::execute_tool`)を「データであり指示ではない」と明示する境界でラップしてからモデルに返すようにした）
- [x] チャットゲートウェイの公開設定デフォルトを「許可リスト制」とする（OpenClawの誤公開インスタンス問題への対策）（4.4で実装済み：`gateway::GatewayConfig::allowed_senders`は既定で空であり、明示的に許可されたsender以外のメッセージはMediatorに到達しない）
