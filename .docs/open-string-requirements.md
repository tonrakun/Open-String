...
- [ ] 権限レベル4段階の定義と切り替えUI
  ...
  - [ ] `middle permission`：ディレクトリ・コマンドのホワイトリスト制。範囲外操作は確認を要求
  - [ ] `high protect`：原則すべての操作に確認を要求。読み取り専用操作のみ自動許可
- [ ] 権限レベルごとの操作ログ記録
- [ ] 危険操作（削除・送信・外部送信・課金）の検出ロジック（権限レベルに依らない共通フィルタ）
...
- [ ] macOS対応（セカンドターゲット）
- [ ] Linux対応（セカンドターゲット）
- [ ] クロスコンパイル/CI構成（単一バイナリビルドをOSごとに自動生成）
