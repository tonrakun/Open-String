...
インストーラは実行ファイルをユーザーディレクトリ配下に配置し、PATHへの追加を行います（冪等）。実行中は各ステップ（バイナリ検出・ディレクトリ作成・コピー・PATH追加・バージョン確認）を番号付きで表示し、完了後にインストール先とバージョンを要約表示します。

アーカイブの代わりに、各OS向けのバイナリ単体（Windows: `open-string-windows-x86_64.exe` / macOS: `open-string-macos-aarch64` / Linux: `open-string-linux-x86_64`）も[Releases](../../releases)から直接ダウンロードできます。

### 更新

既にインストール済みの場合は、再ダウンロード不要でコマンドから更新できます。

```sh
open-string update --check   # 新バージョンの有無だけを確認
open-string update           # 新バージョンがあれば確認の上ダウンロード・置換
open-string update --yes     # 確認プロンプトを省略（非対話実行向け）
```

GitHub Releasesの最新タグと現在のバージョンを比較し、対応する単体バイナリアセットで実行中の自分自身を安全に置き換えます。
