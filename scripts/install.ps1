...
param(
    # Skip the "press Enter to close" pause at the end (used for non-interactive/CI runs).
    [switch]$NoPause
)
...
try {
    ...
    if (-not (Test-Path $sourceExe)) {
        throw "open-string.exe was not found next to this script ($scriptDir). Run this installer from inside the extracted release archive."
    }
